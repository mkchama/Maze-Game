...
#  assembly code into a kernel8.img, which can be run on the Raspberry Pi
...
#  (Windows 10) Specify the directory for QEmu
QEMU_DIRECTORY = C:/"Program Files"/qemu/
QEMU = $(QEMU_DIRECTORY)qemu-system-aarch64.exe
...
#  have to be changed.
INSTALL_DIRECTORY = C:/cygwin64/home/Packages/gcc-linaro-7.3.1-2018.05-i686-mingw32_aarch64-elf/bin/

#  The following are the complete paths to the gcc compiler,
#  the as assembler, the ld linker, and the objcopy and objdump
#  facilities.
GCC = $(INSTALL_DIRECTORY)aarch64-elf-gcc.exe
AS = $(INSTALL_DIRECTORY)aarch64-elf-as.exe
LD = $(INSTALL_DIRECTORY)aarch64-elf-ld.exe
OBJCOPY = $(INSTALL_DIRECTORY)aarch64-elf-objcopy.exe
OBJDUMP = $(INSTALL_DIRECTORY)aarch64-elf-objdump.exe
